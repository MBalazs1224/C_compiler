...
{
    // Do nothing for now, may be implemented later but be careful because there is a high chance of memory corruption
}
...
//Resets iteration, so we start from the start
...
void scope_finish(struct compiler_process* process)
{
    ...
    //If we don't have current scope then we reached back to the root scope
