...
static void lex_finish_expression()
{
    ...
    {
        compiler_error(lex_process->compiler, "You closed an expression you neber opened\n");
    }
}
...
struct token *token_make_multiline_comment()
{
    ...
    while (1)
    {
        ...
        if (c == EOF)
        {
            compiler_error(lex_process->compiler, "You did not close the multiline comment\n");
        }
        else if (c == '*')
        {
            nextc();
...
struct token *token_make_sepcial_number_binary()
{
    // Skip 'b'
    nextc();
...
struct token *token_make_special_number()
{
    ...
    {
        token = token_make_sepcial_number_binary();
    }
...
struct token *read_next_token()
{
    ...
    // We don't care about whitespace so ignore them
    case ' ':
    case '\t':
    ...
    case '\n':
        ...
        // End of File
    case EOF:
        // We have finished lexical analisys on the file
        break;
