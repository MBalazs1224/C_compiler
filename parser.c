...
{
    // The entity flags for the scope entity
    int flags;
...
struct history_cases
{
    ...
    // Is there a default in the switch statement body
...
static void expect_keyword(const char* keyword)
{
    ...
    {
        compiler_error(current_process, "Expected the keyword %s\n", keyword);
    }
}
...
void parser_datatype_init_type_and_size_for_primitive(struct token* datatype_token, struct token* datatype_secondary, struct datatype* datatype_out);
...
void parser_datatype_init_type_and_size_for_primitive(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out)
{
    ...
    {
        compiler_error(current_process,"You are not allowed a secondary datatype for the given datatype%s\n",datatype_token->sval);
    }
...
void parser_datatype_init(struct token* datatype_token, struct token* datatype_secondary_token, struct datatype* datatype_out, int pointer_depth, int expected_type)
{
    ...
    {
        compiler_warning(current_process,"Currently doesn't support 64 bit long long so defaulting to 32 bit long\n");
        datatype_out->size = DATA_SIZE_DWORD;
    }
...
void parser_ignore_int(struct datatype* dtype)
{
    ...
    //'int int a' is not valid so we need to check if int is allowed as secondary
    if (!parser_is_int_valid_after_datatype(dtype))
    {
        compiler_error(current_process,"You provided a secondary \"int\" but it's not allowed as a secondary datatype to this type.");
...
void parse_statement(struct history* history)
{
    ...
    // It must be an expression like x = 50;
    parse_expressionable_root(history);
...
{
    /**
     * Unions share all the memory, so the size of the union is the size of the largest variable inside it
     * union {int a, char b} -> the size will be 4 bytes because the int is the largest
     */
    ...
    *_variable_size += padding;
    // largest_align_eligible_var_node can only be primitive, thats why we need to check if it exists, it will ignore structs and any complex datatype
...
void parse_body_single_statement(size_t* variable_size, struct vector* body_vec, struct history* history)
{
    ...
    // If its a variable than it has to be the largest because there is only one variable
...
{
    // Name and type already parsed (that's why we have dtype), now parsing body and flags
    ...
    if (token_is_identifier(token_peek_next()))
    {
        ...
        {
            //struct {} abc; becomes struct abc {} abc; which can be a problem sometimes (in this way a struct can be declared multiple times without knowing)
            dtype->type_str = var_name->sval;
            dtype->flags &= ~DATATYPE_FLAG_STRUCT_UNION_NO_NAME;
            struct_node->_struct.name = var_name->sval;
        }
        // struct z {} hello -> registers a new variable, thats why we have to register it
        make_variable_node_and_register(history_begin(0),dtype,var_name,NULL);
        // make_variable_node_and_register pushes a new variable node to the stack so we pop that and save it in the struct_node
        struct_node->_struct.var = node_pop();
    }
    // All structs must end with ';'
    expect_sym(';');
...
void parse_union_no_scope(struct datatype* dtype,bool is_forward_declaration) {
    ...
    //If there is a body then it is not a forward declaration
    if (body_node) {
        dtype->size = body_node->body.size;
    }

    // If it has a VAR_NAME i.e
    /*
     * union TYPE_NAME
     * {
     * } VAR_NAME; -> it makes it a variable
     */
    if (token_peek_next()->type == TOKEN_TYPE_IDENTIFIER) {
        struct token *var_name = token_next();
...
bool parse_for_loop_part(struct history* history)
{
    ...
    // Pop off the delimiter ';' thats between parts
    expect_sym(';');
...
void parse_return(struct history* history)
{
    ...
    // Expressionable returned i.e. return 0;
    parse_expressionable_root(history);
    struct node* exp_node = node_pop();
...
void parse_goto(struct history* history)
{
    ...
    expect_sym(';');
    // goto abc; -> abc will be a label node on top of the stack
