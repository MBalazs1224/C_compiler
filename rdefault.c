...
{
    // If the stack offset is negative then we don't need to write the plus (ebp-4)
    if (stack_offset < 0)
    {
...
{
    // If there is one entity, the generator can rely just on the result data
    ...
    // Sets the result addresses based on the base_entity
...
struct resolver_default_entity_data* resolver_default_new_entity_data_for_function(struct node* function_node, int flags)
{
    struct resolver_default_entity_data* entity_data = resolver_default_new_entity_data();
    entity_data->flags = flags;
    entity_data->type = RESOLVER_ENTITY_TYPE_FUNCTION;

    // Functions need global addresses
    resolver_default_global_asm_address(function_node->func.name,0,entity_data->address);
    return entity_data;
}
...
{
    return resolver_new_process(compiler,&(struct resolver_callback){.new_array_entity=resolver_default_new_array_entity,.delete_entity=resolver_default_delete_entity,.delete_scope=resolver_default_delete_scope,.merge_entities=resolver_default_merge_entities,.make_private=resolver_default_make_private,.set_result_base=resolver_default_set_result_base});
}